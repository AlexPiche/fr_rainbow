...
from utils import atari_data
from utils import gym_atari
...
flags.DEFINE_float('reg_weight', 0.75, '')
...
flags.DEFINE_string('algo', 'fr_rainbow2', '')
flags.DEFINE_string('exp_name', 'correct_replay_buffer23', '')
...
def main(argv):
  ...
  name_from_flags = [str(flag) + '_' for flag in [
                                                  FLAGS.exp_name,
                                                  FLAGS.environment_name,
                                                  FLAGS.algo,
                                                  FLAGS.target_network_update_period,
                                                  FLAGS.seed,
                                                  FLAGS.reg_weight,
                                                ]
                  ]
  exp_name = ''.join(name_from_flags)
  ...
  if train_agent.chkp_wab.exists() and train_agent.chkp_wab.stat().st_size > 0:
    wab_id = train_agent.load_wab()
    print(f'> Loaded wandb id ({wab_id}) from {train_agent.chkp_wab}')
  else:
    wab_id = wandb.util.generate_id()
    train_agent.save_wab(wab_id)
    print(f'> New wandb id generated: {wab_id}')

  wandb.init(project=FLAGS.exp_name,
             resume='allow',
             id=wab_id,
             config=FLAGS,
  ...
  state.iteration = train_agent.iteration
  ...
  while state.iteration <= FLAGS.num_iterations:
    ...
    for s in log_output:
      wandb.log({s[0]: s[1], 'frame': state.iteration * FLAGS.num_train_frames,}) 
    
    train_agent.iteration += 1
    state.iteration += 1
    checkpoint.save()
    train_agent.save_state()
