...
class Agent(abc.ABC):
  ...
  def load_wab(self):
    with open(self.chkp_wab, 'r') as wab:
      wab_id = wab.read()
    return wab_id

  def save_wab(self, wab_id):
    with open(self.chkp_wab, "w") as f:
      f.write(wab_id)

  def _load_memory(self):
    with open(self.chkp_mem, 'rb') as data:
      state = pickle.load(data)
      self._replay.set_state(state)

  def _load_state(self):
    with open(self.chkp_file, 'rb') as best_ckpt:
      state = pickle.load(best_ckpt)
      self.set_state(state)

  def save_state(self):
    # Save state
    dict_output = self.get_state() #serialization.to_state_dict()
    with open(self.chkp_file, 'wb') as best_ckpt:
      pickle.dump(dict_output, best_ckpt)

    with open(self.chkp_mem, 'wb') as fh:
      pickle.dump(self._replay.get_state(), fh)
